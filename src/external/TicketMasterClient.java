...
public class TicketMasterClient {
	//send request to TicketMaster to query data
	...
	private static final String API_KEY = "PHpBDgmtAVnKrRi5kNP22obBGZrmjUpx";
	...
	public List<Item> search(double lat, double lon, String keyword) {
		...
		try {
			//ENCODE keyword to UTF-8 (send to TicketMaster; TicketMaster do decode later)
			keyword = URLEncoder.encode(keyword, "UTF-8"); // "Rick Sun" => "Rick%20Sun"
		} catch (UnsupportedEncodingException e) {
			e.printStackTrace();
		}
		
		//assemble query (url + end point + "?" + API key + query parameters)
	...
	// Convert JSONArray event to a list of Item objects.
	...
	//only store image's url -> reduce storage space
	...
	//return set => NO duplicate categories
